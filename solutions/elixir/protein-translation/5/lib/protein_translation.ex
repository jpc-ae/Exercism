defmodule ProteinTranslation do  
  ...
  def of_codon(codon) when byte_size(codon) != 3, do: @invalid_codon
  def of_codon(codon) do
    with {:ok, [protein | []]} <- parse_rna(codon) do
      {:ok, protein}
    else
      {:ok, []} -> {:ok, "STOP"}
      _ -> @invalid_codon
  ...
  defp parse_rna(_, _), do: @invalid_rna
end